...
class Paging {
	...
	function string_paging() {
		...
		$end_page=$start_page+6; /* custom by Hung; default 4 */
		if($end_page>$pages_required) { $end_page=$pages_required; }
		if(($this->show_first_last)&($this->current_page>5)) {
			if(!empty($this->action_path)) {
				$page_line.='<li ><a page="'.(1).'" href="'.str_replace("{i}", "1", $this->action_path).'">Đầu</a></li>';
			} else {
				$page_line.='<li ><a page="'.(1).'" href="'.$this->action."?".$this->parameter_name.'='.
					 "1".'">Đầu</a></li>';
			}
		}
		/* custom by Hung */
		...
		if($start_page>5) {
			if(!empty($this->action_path)) {
				$page_line.='<li ><a page="'.($start_page-1).'" href="'.str_replace("{i}", $start_page-1, $this->action_path).'"'.$link_class.'>Trước</a></li>';
			} else {
				$page_line.='<li ><a page="'.($start_page-1).'" href="'.$this->action."?".$this->parameter_name.'='.($start_page-1).'" '.$link_class.'>Trước</a></li>';
			}
		}
		...
		for($i=$start_page; $i<=$end_page; $i++) {
			...
			if($this->current_page!=$i) {
				if(!empty($this->action_path)) {
					$page_line.='<li ><a page="'.($i).'" href="'.str_replace("{i}", $i, $this->action_path).'">'.$i.'</a></li>';
				} else {
					$page_line.='<li ><a page="'.($i).'" href="'.$this->action."?".$this->parameter_name.'='.
						 $i.'">'.$i.'</a><li>';
				}
			} else {
				$page_line.='<li><a href="" class="active">'.$i.'</a></li>';
			}
		...
		if($pages_required>$end_page) {
			if(!empty($this->action_path)) {
				$page_line.='<li ><a page="'.($end_page+1).'" href="'.str_replace("{i}", $end_page+1, $this->action_path).'"'.$link_class.'>Tiếp</a></li>';
			} else {
				$page_line.='<li ><a page="'.($end_page+1).'" href="'.$this->action."?".$this->parameter_name.'='.($end_page+1).'" '.$link_class.'>Tiếp</a></li>';
			}
		}
		...
		if(($this->show_first_last)&($pages_required>$end_page)) {
			if(!empty($this->action_path)) {
				$page_line.='<li ><a page="'.$pages_required.'" href="'.str_replace("{i}", $pages_required, $this->action_path).'"'.$link_class.'>Cuối</a></li>';
			} else {
				$page_line.='<li ><a page="'.$pages_required.'" href="'.$this->action."?".$this->parameter_name.'='.
					 $pages_required.'"'.$link_class.'>Cuối</a></li>';
			}
		}
	...
	function set_link_string($first="Đầu tiên", $back="Trang trước", $next="Trang sau", $last="Cuối cùng"){
