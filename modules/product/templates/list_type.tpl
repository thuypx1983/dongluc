    ...
    <div class="content_cate"> <a href="{$url}san-pham/" class="head_title">Danh mục sản phẩm</a>
      <ul class="pkg list_menu_product">
        {foreach from=$rows item=row key=k}
            ...
            <li><a href="{if $row.link_to!=''}{$row.link_to}{else}{$url}{$row.link}-{$row.id}/{/if}"><span class="name_cate">{$row.title}</span><img src="{$url}upload/category/thumb/{$row.thumb|default:'no-img.jpg'}"/> </a></li>
        {foreachelse}
