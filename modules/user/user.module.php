	...
	*/
	...
	function authentication()
	{
		if(empty($_SESSION["user"]))
			header("location: ".SITE_URL."dang-nhap.html");
	}
	...
	function history() {
		...
		$sql = "
				select 
				t1.title, t1.photo, t1.create_date,
			...
			";
		$orders = $this->db->getAll("SELECT id, cust_note, is_process, create_date FROM orders WHERE user_id = {$_SESSION['user']['id']} ORDER BY id DESC");
		...
		foreach ($orders as $k => $v) {
			$sql = "
				...
				WHERE t2.order_id = '{$v['id']}' ";
			$orders[$k]['sub'] = $this->db->getAll($sql);
	...
	function register()
	{
		#echo '<pre>'; print_r($_POST); echo '</pre>';
		if(!empty($_SESSION["user"]))
			header("location: ".SITE_URL."don-hang.html");

		if($_SERVER["REQUEST_METHOD"]=="POST" && $_POST["submit_login"]!='')
		{
			$this->login();
		}
		
		if($_SERVER["REQUEST_METHOD"]=="POST" && $_POST["submit_register"]!='')
		{	
			...
			{
				$user = $this->db->getRow("select * from user where email='{$_POST['register_email']}'");
				if(count($user)>0 && $user['email']== $_POST["register_email"]) {
					...
					$row= array(
						...
						"password" => sha1(md5(PASSSWORD_HASH.$_POST["register_password"])),
						"is_active" => 0,
					...
					if(mysql_error()!='')
					{
						...
						$content= "Xin chào: <b>{$_POST['register_email']}</b><br><br>";
						$content.= "Bạn vừa đăng ký tài khoản trên ".SITE_ALIAS.", vui lòng click vào link dưới đây để kích hoạt tài khoản của bạn:<br/><br/>";
						$content .= "<a href='".SITE_URL."?mod=user&task=active&code=".md5($id)."&email=".base64_encode($_POST['register_email'])."'>".SITE_URL."?mod=user&task=active&code=".md5($id)."&email=".base64_encode($_POST['register_email'])."</a><br/><br/>";
						...
						if(!sendMail($email, $email, $subject, $content)) {
							
							$msg_error = "Đăng ký tài khoản thành công<br />Tuy nhiên đã xảy ra lỗi trong quá trình gửi mail <a href=''>Thử lại</a>";
						}
						else {
							$msg_success = "Đăng ký tài khoản thành công<br />Mail xác nhận kích hoạt đã được gửi tới hòm thư: <b>".$email."</b> của bạn. Vui lòng kiểm tra email để kích hoạt tài khoản";
						}
					}
				}
			}		
	...
	function login() {
		...
		if(strcasecmp($_SESSION['key_captcha'], $_POST['login_captcha']))
		{
			$msg_error = "Sai mã bảo mật";
		}
		else
		{
			$user= $this->db->getRow("select * from user where email='".$_POST["login_email"]."'");
			if(count($user)>0)
			{									
				if($user["password"]==sha1(md5(PASSSWORD_HASH.$_POST["login_password"])))
				{
					if($user["is_active"]!="1") {
		...
		$this->smarty->assign("msg_error_login", $msg_error);
		$this->smarty->assign("msg_success_login", $msg_success);
	}
	...
	function getPageinfo($task="")
	{
		...
		switch($task)
		{
			case "update_profile": 
				...
				$page["description"]= "Thông tin cá nhân";
				$page["keyword"]= "Thông tin cá nhân";
				break;
			case "history": 
				$page["title"]= "Đơn hàng của tôi";
				$page["description"]= "Đơn hàng của tôi";
				$page["keyword"]= "Đơn hàng của tôi";
				break;
			case "login": 
			...
			case "password": 
				...
				break;
			case "active": 
				$page["title"]= "Kích hoạt tài khoản trên ".SITE_ALIAS;
				$page["description"]= "Kích hoạt tài khoản trên ".SITE_ALIAS;
				$page["keyword"]= "Kích hoạt tài khoản trên ".SITE_ALIAS;
				break;
			case "list": 
				$page["title"]= "ten nhom";
